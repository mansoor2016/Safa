...
- **Settings proximity**: Settings for a feature should live as close as possible to the functionality they control. Prefer inline controls, context menus, and long-press options on the feature's own screen over burying configuration in the global Settings tab. Global Settings is for app-wide preferences (calculation method, appearance, notifications). Feature-specific options (auto-scroll speed, progress ring mode, font size) belong on or adjacent to the feature itself. This reduces navigation friction and makes discovery natural.
